...
|---|---|---|---|---|
| `{{FIRMA_NAME}}` | Name des Unternehmens bzw. des Inhabers | Valtix Financial Management | impressum.html:79, datenschutz.html:80 | Pflicht |
| `{{RECHTSFORM}}` | Rechtsform | GmbH / UG (haftungsbeschränkt) / Einzelunternehmen / GbR | impressum.html:79, datenschutz.html:80 | Pflicht |
| `{{VERTRETUNGSBERECHTIGTE_PERSON}}` | Vertretungsberechtigte Person(en), z. B. Geschäftsführung oder Inhaber | Vorname Nachname | impressum.html:84, datenschutz.html:81 | Pflicht |
...
| `{{REGISTERGERICHT}}` | Registergericht | Amtsgericht Leipzig | impressum.html:94 | Optional, nur bei Registereintrag |
| `{{REGISTERNUMMER}}` | Handels-/Registernummer | HRB 12345 | impressum.html:95 | Optional, nur bei Registereintrag |
| `{{UST_IDNR}}` | Umsatzsteuer-Identifikationsnummer nach § 27a UStG | DE123456789 | impressum.html:100 | Optional, nur falls vergeben |
| `{{INHALTLICH_VERANTWORTLICHER}}` | Verantwortliche Person nach § 18 Abs. 2 MStV | Vorname Nachname | impressum.html:105 | Pflicht |
...
- **Registereintrag/USt-IdNr. (impressum.html):** Die Abschnitte samt Hinweiskästen ersatzlos
  löschen, falls nicht zutreffend; die grauen Hinweiskästen (`class="note"`) sind Arbeitshilfen
  und sollten vor dem Livegang in jedem Fall entfernt werden.
